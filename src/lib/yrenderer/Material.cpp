...
void ShaderCache::_prepare_shader(RenderPathType render_path_type, const Material* material, const string& vertex_module, const string& geometry_module, const string& tessellation_module) {
	...
	shader[i] = ctx->shader_manager->load_surface_shader(material->pass0.shader_path, rpt, vertex_module, geometry_module, tessellation_module)
		.value_or(ctx->shader_manager->dummy_surface_shader());
}
