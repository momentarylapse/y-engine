...
AccelerationStructure::AccelerationStructure(const VkAccelerationStructureTypeNV type, const Array<VkGeometryNV> &geo, const uint32_t instanceCount) {
	...
	_vkGetAccelerationStructureMemoryRequirementsNV(default_device->device, &memory_requirements_info, &memoryRequirements);

	VkMemoryAllocateInfo memory_allocate_info = {};
	memory_allocate_info.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
	memory_allocate_info.allocationSize = memoryRequirements.memoryRequirements.size;
	memory_allocate_info.memoryTypeIndex = default_device->find_memory_type(memoryRequirements.memoryRequirements, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

	error = vkAllocateMemory(default_device->device, &memory_allocate_info, nullptr, &memory);
	if (VK_SUCCESS != error)
	...
	error = _vkBindAccelerationStructureMemoryNV(default_device->device, 1, &bind_info);
	if (VK_SUCCESS != error)
...
AccelerationStructure *AccelerationStructure::create_bottom(VertexBuffer *vb) {
	...
	auto as = new AccelerationStructure(VK_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL_NV, geo, 0);
	as->build(geo, {});
	return as;
}
...
AccelerationStructure *AccelerationStructure::create_top(const DynamicArray &instances) {
	...
	auto as = new AccelerationStructure(VK_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL_NV, {}, instances.num);
	as->build({}, instances);
	return as;
}
