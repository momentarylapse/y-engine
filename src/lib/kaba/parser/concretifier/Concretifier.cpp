...
const Class *give_useful_type(Concretifier *con, shared<Node> node) {
	if (node->type == common_types.unknown)
...
shared<Node> Concretifier::link_special_operator_is(shared<Node> param1, shared<Node> param2, int token_id) {
	...
	if (t1->is_some_pointer()) {
		// FIXME is this safe?
		param1->type = tree->type_ref(common_types.pointer, token_id);
		param1 = param1->deref();
		t1 = t1->param[0];
	}
	...
	// vtable2
	auto vtable2 = add_node_const(tree->add_constant_pointer(common_types.pointer, t2->_vtable_location_compiler_), token_id);
...
shared<Node> Concretifier::link_special_operator_tuple_extract(shared<Node> param1, shared<Node> param2, int token_id) {
	...
	auto node = new Node(NodeKind::TupleExtraction, -1, common_types._void, Flags::None, token_id);
		...
		if (param1->kind == NodeKind::CallFunction) {
			...
			if (f->name == Identifier::func::Get) {
				...
				for (auto *ff: weak(inst->type->functions))
					if (ff->name == Identifier::func::Set and ff->literal_return_type == common_types._void and ff->num_params == 3) {
...
shared<Node> Concretifier::type_to_value(shared<Node> node) {
	if (node->kind == NodeKind::Class)
		return add_node_const(tree->add_constant_pointer(common_types.class_ref, node->as_class()), node->token_id);
	return node;
}
...
shared<Node> Concretifier::concretify_call(shared<Node> node, Block *block, const Class *ns) {
	...
	for (auto&& [i,l]: enumerate(weak(links))) {
		...
		/*} else if (l->type == common_types.functionCodeP) {
			auto c = new Node(NodeKind::POINTER_CALL, 0, common_types._void);
			c->set_num_params(1);
			c->set_param(0, l);
	...
	if (operand->kind == NodeKind::Function) {
		...
		for (auto l: weak(links)) {
			...
			if (auto ff = context->template_manager->request_function_instance(tree, f, tt, node->token_id)) {
				auto tf = add_node_func_name(ff, node->token_id);
...
shared_array<Node> Concretifier::concretify_token(shared<Node> node, Block *block, const Class *ns) {
	...
	if (auto s = parser->which_special_function(token)) {
		...
		return {add_node_special_function_name(s->id, node->token_id, common_types.special_function_ref)};
	}
...
shared<Node> Concretifier::concretify_var_declaration(shared<Node> node, Block *block, const Class *ns) {
	...
	//as_const
	auto create_var = [block, &node, this] (const Class* type, Node* name) {
		...
		return block->add_var(name->as_token(), type, name->token_id);
	};
	Array<Variable*> vars;
	...
	if (node->params[1]->kind == NodeKind::Tuple) {
		...
		for (auto&& [i,t]: enumerate(etypes))
			vars.add(create_var(t, node->params[1]->params[i].get()));
	} else {
		vars.add(create_var(type, node->params[1].get()));
	}
	...
	if (node->params.num == 3) {
		...
		if (is_non_owning_pointer(type)) {
			if (rhs->type != vars[0]->type)
				if (rhs->type != common_types.none)
					do_error(format("pointer initialization type mismatch '%s = %s'", vars[0]->type->long_name(), rhs->type->long_name()), rhs);
			node = add_node_operator_by_inline(InlineID::PointerAssign, add_node_local(vars[0], node->token_id), rhs, node->token_id);
		} else {
...
shared<Node> Concretifier::concretify_array_builder_for_inner(shared<Node> n_for, shared<Node> n_exp, shared<Node> n_cmp, const Class *type_el, Block *block, const Class *ns, int token_id) {
	...
	auto array_var = block->add_var(block->function->create_slightly_hidden_name(), array_type, n_for->token_id);

	// array.add(exp)
	auto f_add = array_type->get_member_func("add", common_types._void, {type_el});
	if (!f_add)
		do_error("...add() ???", token_id);
	auto n_add = new Node(NodeKind::AbstractCall, 0, common_types.unknown, Flags::Mutable, token_id);
	n_add->set_num_params(3);
	n_add->set_param(0, add_node_func_name(f_add, n_exp->token_id));
	n_add->set_param(1, add_node_local(array_var, n_exp->token_id));
	...
	n->set_num_params(2);
	n->set_param(0, n_for);
	n->set_param(1, add_node_local(array_var, n_exp->token_id));
	return n;
}
...
shared<Node> Concretifier::concretify_node(shared<Node> node, Block *block, const Class *ns) {
	...
	if (node->kind == NodeKind::AbstractOperator) {
		...
		for (int p=0; p<node->params.num; p+=2) {
			if (node->params[p]->type != common_types.string)
...
shared<Node> Concretifier::concretify_slice(shared<Node> node, Block *block, const Class *ns) {
	...
	if (!step) {
		if (type) {
			step = add_node_const(tree->add_constant_int(1), node->token_id);
		} else {
			step = add_node_const(tree->add_constant(common_types.f32, node->token_id), node->token_id);
...
const Class *type_more_dominant(const Class *a, const Class *b) {
	...
	if (is_x(common_types.i32, common_types.f32))
		return common_types.f32;
	if (is_x(common_types.i32, common_types.f64))
		return common_types.f64;
	if (is_x(common_types.i32, common_types.i64))
		return common_types.i64;
	if (is_x(common_types.i64, common_types.f64))
		return common_types.f64;
	return nullptr;
}
...
shared<Node> Concretifier::force_concrete_type(shared<Node> node) {
	...
	if (node->kind == NodeKind::ArrayBuilder) {
		if (node->params.num == 0) {
			node->type = common_types.i32_list;
			return node;
		}
		...
		if (node->params.num == 0) {
			node->type = common_types.i32_dict;
			return node;
		}
...
shared<Node> SyntaxTree::make_fake_constructor(const Class *t, const Class *param_type, int token_id) {
	//if ((t == common_types.i32) and (param_type == common_types.f32))
	...
	auto *dummy = new Node(NodeKind::Placeholder, 0, common_types._void);
...
shared_array<Node> Concretifier::turn_class_into_constructor(const Class *t, const shared_array<Node> &params, int token_id) {
	if (((t == common_types.i32) or (t == common_types.f32) or (t == common_types.i64) or (t == common_types.f64) or (t == common_types._bool) or (t == common_types.i8) or (t == common_types.u8)) and (params.num == 1))
	...
	if (links.num == 0) {
		for (auto *cf: t->get_constructors()) {
			msg_write(cf->signature(common_types._void));
...
void Concretifier::concretify_function_body(Function *f) {
	int n0 = f->block_node->params.num;
	int i0 = 0;
	...
	for (int i=0; i<f->block_node->params.num - n0; i++) {
			...
			for (int i=0; i<f->num_params; i++)
				if (!params[i]) {
					if (i >= f->mandatory_params and i < f->param_default_values.num and f->param_default_values[i]) {
						params[i] = cp_node(f->param_default_values[i], nullptr, operand->token_id);
					} else {
...
shared<Node> Concretifier::build_lambda_template(const shared<Node>& param, const shared<Node>& expression, Block *block, const Class* ns, int token_id) {
	...
	node->set_param(1, add_node_class(common_types.unknown, token_id));
	...
	auto ret = add_node_statement(StatementID::Return, token_id, common_types.unknown);
	...
	return add_node_func_name(f, token_id);
}
...
shared<Node> Concretifier::apply_params_with_cast(shared<Node> operand, const shared_array<Node> &params, const CastingDataCall &casts, int offset) {
	...
	if (operand->is_function()) {
		...
		for (int p=params.num+offset; p<f->param_default_values.num; p++) {
			r->set_param(p, cp_node(f->param_default_values[p], nullptr, operand->token_id));
		}
	}
	return r;
}
...
shared<Node> Concretifier::add_converter_str(shared<Node> node, bool as_repr) {
	...
	Function *cf = nullptr;
	if (as_repr)
		cf = t->get_member_func(Identifier::func::Repr, common_types.string, {});
	if (!cf)
		cf = t->get_member_func(Identifier::func::Str, common_types.string, {});
	if (cf)
		return add_node_member_call(cf, node, node->token_id);

	// "universal" var2str() or var_repr()
	auto *c = tree->add_constant_pointer(common_types.class_ref, t);
	...
	cmd->set_param(1, add_node_const(c, node->token_id));
	return cmd;
}

shared<Node> Concretifier::make_dynamical(shared<Node> node) {
	if (node->kind == NodeKind::ArrayBuilder and node->type == common_types.unknown) {
		for (int i=0; i<node->params.num; i++)
			node->params[i] = make_dynamical(node->params[i].get());
		// TODO create...
		node->type = common_types.any_list;
		//return node;
	} else  if (node->kind == NodeKind::DictBuilder and node->type == common_types.unknown) {
		for (int i=1; i<node->params.num; i+=2)
			node->params[i] = make_dynamical(node->params[i].get());
		// TODO create...
		node->type = common_types.any_dict;
		//return node;
	}
	//node = force_concrete_type(tree, node);

	auto *c = tree->add_constant_pointer(common_types.class_ref, node->type);
