...
void Camera::start_script(const string &filename,const vector &dpos)
{
#if 0
	if (filename.num <= 0){
		automatic=false;
...
void Camera::start()
{
	...
	nix::SetOverrideShader(override_shader);
#endif
}
...
/*	for (int i=0;i<num_used_clipping_planes;i++)
	...
	float center_x = (float)vulkan::target_width * (dest.x1 + dest.x2) / 2;
	float center_y = (float)vulkan::target_height * (dest.y1 + dest.y2) / 2;
	float height = (float)vulkan::target_height * (dest.y2 - dest.y1) / zoom;
	...
	m_projection = matrix::perspective(pi/4, vulkan::target_width / (float)vulkan::target_height, min_depth, max_depth);
...
/*	float center_x = (float)nix::target_width * (dest.x1 + dest.x2) / 2;
	...
	m_projection = matrix::perspective(pi/4, vulkan::target_width / (float)vulkan::target_height, min_depth, max_depth);
	m_view = (matrix::translation(pos) * matrix::rotation_q(ang)).transpose();
